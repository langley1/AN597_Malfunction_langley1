...
bro<- function(x) {ifelse (x=="bro", "bro", "*blank stare*")}
bro("bro")    
...
ztest<- function(m,sem,alpha) {}

function(p1, p0, n1, n2=NULL, p2=NULL, a=0.05, alternative = "two.sided",)
{
    if (alternative == "two.sided")
        return((p1 - p0)/(sqrt(p1 * q/ n1)))
...
lower <- m - qnorm(1 - a/2) * sem  # (1-alpha)/2 each in the upper and lower tails of the distribution
upper <- m + qnorm(1 - a/2) * sem  # (1-alpha)/2 each in the upper and lower tails of the distribution
...
edit(lm) and look at this function, use the same layout!!!
...
```{r}
###I dont think this is right.... delete this chunk!?
df<- data.frame(cbind(KC$Brain_Size_Species_Mean, KC$MaxLongevity_m)) #making my lm1 into a data frame so that I can rename the column names
head(df)

names(df)<- c("mean_bs", "max_l") #changing the column names
head(df)

df<- cbind(df, lm1_pi)
head(df)

ci_df<- data.frame(cbind(confint(lm1, level = 0.90)))
head(ci_df)

lm1_pi<- predict(lm1, interval = "prediction", level = 0.90)
lm1_pi
```
...
```{r}
plot3 <- ggplot(data = KC, aes(x = mean_bs, y = max_l))
plot3 <- plot3 + labs(x= "Mean Brain Size", y= "Longevity")
plot3 <- plot3 + geom_point(alpha = 1/2)
plot3 <- plot3 + geom_line(aes(x = mean_bs, y = CIfit), colour = "black")
g <- g + geom_line(aes(x = mean_bs, y = lm1_ci), colour = "blue")
g <- g + geom_line(aes(x = mean_bs, y = CIupr), colour = "blue")
...
#Part 2, Question 3: adding in a legend to differentiate between prediction intervals and confidence intervals