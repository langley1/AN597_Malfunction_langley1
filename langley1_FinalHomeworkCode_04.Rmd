...
I put my function in #s so that my Rmd would knit (it wouldn't knit otherwise because there are errors in my function)
```{r}
...
##This is version 1: I just manually type in the equation using the label() function on line 125
...
##This is version 2: I'm trying package "basicTrendline" to add in the equation for me
library(basicTrendline)

#plot1 <- ggplot(data = KC, aes(x = mean_bs, y = max_l))
#plot1 <- plot1 + labs(x= "Mean Brain Size", y= "Longevity")
#plot1 <- plot1 + geom_point()
#plot1 <- plot1 + geom_smooth(method = "lm", formula = y ~ x, se=FALSE) #geom_smooth puts in 95% confidence intervals automatically into the graph, by setting se = F, you can remove these CI
#plot1 <- plot1 + trendline(mean_bs, max_l, model = "line2P", show.equation = TRUE, linecolor = "red", CI.level = 0.90, CI.fill= TRUE, show.Rpvalue = FALSE)
#plot1

#This version didn't work properly and kept giving me errors, I've put #s so that I can knit it but keep the code as reference
```
...
#leg.text<- c("90% PI", "90% CI") #trying out the legend() function to see if it will add in the legend for me
#plot3(legend(list(x=200, y=800), legend = leg.text, col = 2:3, pch = 1:2, lty = 1)) #UGH this also doesn't work... 
...
plot4<- ggplot(new_df_log, aes(log(mean_bs), log(max_l)))+
    geom_point()+
...
* Part 2, Question 1: Still not 100% confident with adding the equation to my plots. I tried the basicTrendline package, which you can see in my version two on lines 110 and 140.
